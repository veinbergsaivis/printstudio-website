...
const StatItem: React.FC<StatItemProps> = ({ valueKey, labelKey }) => {
  ...
  return (
    <div
      ...
      role="listitem"
    >
      <span className="block text-2xl md:text-3xl font-bold text-text-base">
        {t(valueKey, '0')}
      </span>
      <p className="text-sm text-text-muted">{t(labelKey, labelKey.split('.').pop())}</p>
    </div>
  );
};
  ...
  return (
    ...
    >
      ...
      <h3 className="text-xl font-semibold text-text-base mb-2">
        {t(titleKey, titleKey.split('.').pop())}
      </h3>
      <p className="text-text-muted text-sm leading-relaxed">
        {t(descriptionKey, descriptionKey.split('.').pop())}
      </p>
    </div>
  );
};
...
const AboutSection: React.FC = () => {
  ...
  const stats = [
    ...
    { id: 'projekti', valueKey: 'about.stats.projekti.value', labelKey: 'about.stats.projekti.label' },
    { id: 'apmierinatiba', valueKey: 'about.stats.apmierinatiba.value', labelKey: 'about.stats.apmierinatiba.label' },
  ];

  return (
    <section id="about" className="py-16 md:py-24 bg-background" aria-labelledby="about-heading">
      <div className="container mx-auto px-4 md:px-8">
        <div className="text-center mb-12 md:mb-16">
          <h2 id="about-heading" className="text-3xl md:text-4xl lg:text-5xl font-bold text-text-base mb-4">
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-start">
          ...
          <div className="order-2 lg:order-1 space-y-6">
            ...
            {/* Statistika */}
            <div className="flex flex-col sm:flex-row justify-center lg:justify-start gap-4 pt-2" role="list" aria-label={t('about.stats.aria', 'Statistika')}>
              {stats.map((stat) => (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-4">
              <div className="bg-surface border border-border-color rounded-xl p-5">
                ...
                <ul className="list-disc list-inside text-sm text-text-muted space-y-1">
                  ...
                  <li>{t('about.buckets.b2b.items.2', 'Sērijveida pasūtījumi un noliktavas papildinājumi')}</li>
                  <li>{t('about.buckets.b2b.items.3', 'Dāvanu komplekti klientiem un darbiniekiem')}</li>
                </ul>
              </div>
              <div className="bg-surface border border-border-color rounded-xl p-5">
                <h3 className="text-lg font-semibold text-text-base mb-3">
                ...
                <ul className="list-disc list-inside text-sm text-text-muted space-y-1">
                  <li>{t('about.buckets.b2c.items.0', 'Personalizētas dāvanas un vienas vienības druka')}</li>
          ...
          <div className="order-1 lg:order-2">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {features.map((feature) => (
                <FeatureCard key={feature.id} icon={feature.icon} titleKey={feature.titleKey} descriptionKey={feature.descriptionKey} />
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="mt-12 text-center">
          <p className="text-text-muted mb-4">
            {t(
              'about.cta.subtitle',
              'Nosūti savu ideju — atbildēsim ar cenu un termiņu.'
            )}
          </p>
          <a
            href="#contact"
