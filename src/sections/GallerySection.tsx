...
const GallerySection: React.FC = () => {
  ...
  return (
    <section id='gallery' className='relative py-16 md:py-24 bg-surface'>
          ...
          {categories.map(category => (
            ...
            >
              {t(
                ...
                category === 'all' ? t('gallery.categories.all', 'All') : category
              )}
            </Button>
          ))}
        </div>
            ...
            {filteredItems.map(item => (
              ...
              >
                <GalleryItem
                  image={item.image}
                  title={t(`gallery.items.${item.id}.title`, item.id)}
                  description={t(`gallery.items.${item.id}.description`, 'Apraksts nav atrasts')}
