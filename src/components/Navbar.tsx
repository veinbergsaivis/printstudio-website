...
const sectionIds = ['home', 'about', 'services', 'gallery', 'testimonials', 'blog', 'faq', 'contact'] as const
const pageLinks = [
...
const NavLink: React.FC<NavLinkProps> = ({ children, onClick, isActive, className }) => {
  return (
    <button
      className={cn(
        "relative font-medium group transition-colors duration-200 px-3 py-1.5 rounded-lg text-sm leading-6",
    ...
    >
      ...
      <span
        className={cn(
          ...
          // neaktīvajam
          !isActive &&
            "w-0 opacity-0 group-hover:w-[45%] group-hover:opacity-100",
          // aktīvajam
    ...
    >
      ...
      >
        <div className={cn('flex items-center justify-between', isScrolled ? 'h-12' : 'h-16')}>
          ...
          >
            {sectionIds.map(id => (
              <NavLink key={id} onClick={() => handleSectionClick(id)} isActive={isHomePage && activeSection === id}>
            ...
            {pageLinks.map(link => (
              <NavLink
                ...
                isActive={link.path === '/pricing' ? isPricingActive : location.pathname === link.path}
              >
      ...
      >
        <div className="space-y-2 px-4 pt-2 pb-4">
          ...
          <div className="my-2 border-t border-black/80" />
          {pageLinks.map(link => (
            <NavLink
              key={link.id}
              onClick={() => handleRouteClick(link.path)}
